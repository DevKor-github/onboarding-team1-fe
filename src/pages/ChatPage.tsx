...
export const ChatPage = () => {
  ...
  useEffect(() => {
    if (websocket.message) {
      ...
      if (chatData.messageType === 'TALK' && chatData.senderId !== 1) setTextArray([...textArray, { text: chatData.message, time: date, type: 'OTHER' }]);
    }
  }, [websocket.message]);
    ...
    <div className="flex h-screen w-1/3 min-w-96 max-w-2xl flex-col border border-black">
      ...
      <div className="scrollbar-hide flex-1 overflow-y-scroll">
        {textArray.map((chat, index) => {
          ...
          else return <OtherChatBox profileImgUrl={AVATAR_URL} style={style} text={chat.text} time={date} key={'otherchat' + index} />;
        })}
      </div>
      <ChatInputField websocket={websocket} textArray={textArray} setTextArray={setTextArray} />
    </div>
  );
};
